...
my $testfile = "TranslitMyanmarDCLexamples.tsv"; 
open(my $input, '<:encoding(utf8)', $testfile) or die "Could not open file '$testfile' $!";

print "#myanmar\tDCLstandard\tOBI\n";

while (my $line = <$input>) {
	chomp $line;
	if ($line =~ m/^#.*/){next;}
	my @examples = split('\t', $line);
	my $burmese = $examples[0];		
	my $standard = $examples[1];
	my $simplified = $examples[2];
	
	my $test = transliterate($burmese);
	print "$burmese\t$standard\t";
	if ($test eq $standard){
		print "$test \n";
	}else{
		print "$test\n";
