...
def getNextTransBreak(s, idx):
    ...
    while curidx < slen:
        ...
        if state is None:
            ...
            continue
        res = AUTO[state][cl]
        print("trans = %s, resrest = %s , %d + %d = %d " % (ress, resrest, state, cl, res))
        if res > 5:
            print("test %s" % res)
            state = res
            curidx += nbchars
        ...
        if res == 0:
            ...
            lastrepl = repl
            continue
        print("return with trans=%s res=%d, idx=%d" % (ress, res, idx+res-1))
